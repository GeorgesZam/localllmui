...
class DocumentParser:
    ...
    def _parse_pdf(self, file_path: str) -> str:
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                ...
                for page_num, page in enumerate(reader.pages):
                    try:
                        ...
                        # If we got meaningful text (more than just whitespace/noise)
                        if len(page_text) > 50:
                            text_parts.append(f"=== Page {page_num + 1} ===\n{page_text}")
                        else:
                            # Page seems to be scanned/image-based
                            scanned_pages.append(page_num + 1)
                            print(f"[Parser] Page {page_num + 1} appears to be scanned (only {len(page_text)} chars)")
                    except Exception as e:
            ...
            # If we have scanned pages and OCR is available, try OCR on the whole PDF
            if scanned_pages:
            ...
            # If we still have no text, the PDF might be entirely image-based
            if not text.strip() and total_pages > 0:
                print("[Parser] No text extracted, PDF may be entirely image-based")
                if self.ocr.available and self.ocr.pdf_support:
                    print("[Parser] Attempting full OCR...")
